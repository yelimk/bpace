...
- **측정 결과 화면 (measurement_result_screen)**: 생체 수치(BPM·HRV·컨디션 점수) 변동폭 기반 4단계 맞춤형 심박 분석 리포트 실시간 생성
...
### 측정 원리
손가락을 스마트폰 카메라와 LED 플래시에 밀착하면, 심장 박동에 따른 피하 미세혈관의 혈류량 변화로 헤모글로빈의 적색 광 흡수율이 미세하게 변동합니다. 초당 30프레임으로 연속 수집한 20초 파형 데이터(600샘플)로 BPM·SDNN·RMSSD·컨디션 점수를 산출합니다. 컨디션 점수는 BPM 정규화 점수(40%)와 HRV 정규화 점수(60%)를 가중 합산하여 0~100점으로 환산합니다.

### 컨디션 점수별 상태 및 추천 루틴

| 점수 구간 | 상태 | 추천 카테고리 | 추천 루틴 |
| :---: | :--- | :---: | :--- |
| 85 ~ 100점 | 최상 (Optimal) — 자율신경 밸런스 최상 | 회복 | 5-5 공진 호흡, 2-1-4-1 횡격막 복식호흡 |
| 70 ~ 84점 | 양호 (Good) — 전반적 안정 / 몰입 추천 | 집중 | 4-4-4-4 박스 호흡, 4-2-4-2 세미 박스 호흡 |
| 50 ~ 69점 | 이완 필요 (Calm Needed) — 피로/긴장 축적 | 이완 | 4-7-8 호흡, 4-6 릴렉스 호흡 |
| 0 ~ 49점 | 경고 (High Stress) — 급성 긴장/스트레스 | 진정 | 생리학적 한숨 |
| — | 중요 일정 30분 전 (캘린더 상황 감지) | 각성 | 4-1-2-1 각성 호흡 |
