...
5. [📅 5. 구글 캘린더 일정 융합 및 상황 인지(Context-Aware) 알고리즘](#-5-구글-캘린더-일정-융합-및-상황-인지context-aware-알고리즘)
...
## 📅 5. 구글 캘린더 일정 융합 및 상황 인지(Context-Aware) 알고리즘

BPACE 시스템은 단순 생체 지표만 수동으로 보는 것을 넘어, **사용자의 실제 캘린더 일정 상황(Google Calendar API)**을 백엔드에서 융합하여 미팅/발표 전 최상의 상태로 준비하도록 돕는 **'선제적 리추얼 흐름(Pre-Event Ritual Flow)'**을 제공합니다.

```mermaid
graph TD
    Cal["구글 캘린더 일정 동기화"] --> CheckSchedule{"현재 시각 기준 30분 이내에<br/>중요 일정/미팅이 존재하는가?"}
    CheckSchedule -- 예 (일정 30분 전) --> Push["🔔 [1단계] 선제적 푸시 알림<br/>'30분 뒤 일정이 있어요! 리추얼로 준비해 볼까요?'"]
    Push --> Click["📱 [2단계] 알림 클릭 ➡️ '일정 대비 준비하기' 화면 진입"]
    Click --> PPG["📸 [3단계] 실시간 카메라 생체 측정 (BPM / HRV 산출)"]
    PPG --> Rec["🫁 [4단계] 일정 상황 + 실시간 생체 데이터 융합 호흡 추천"]
    CheckSchedule -- 아니오 --> Standard["일반 생체 측정 모드 (BPM / HRV 생체 표준 트리)"]
```

### 🎯 캘린더 연동 리추얼 UX & 추천 알고리즘 상세 흐름

#### 1️⃣ [1단계] 선제적 리추얼 푸시 알림 (Proactive Ritual Notification)
* **트리거**: 캘린더 일정 시작 **30분 전** 백엔드 알림 스케줄러 작동 (기본값).
* **알림 타임라인 옵션**: 사용자 앱 설정에서 `10분 전`, `15분 전`, `30분 전(기본값)` 중 알림 시점을 자율 선택 가능.
* **알림 문구**: *"30분 뒤 [중요 미팅] 일정이 있습니다. 리추얼로 마음을 준비해 볼까요?"*

#### 2️⃣ [2단계] '준비하기' 화면 진입 & 카메라 측정 (Pre-Event Measurement)
* 사용자가 알림을 터치하면 앱 내 **'일정 대비 준비하기(Ritual Prep)' 화면**으로 즉시 진입합니다.
* 사용자는 손가락을 카메라에 대고 20초간 **실시간 생체 파형(PPG)**을 측정합니다.

### 🎯 캘린더 일정 분류 기준 및 융합 의사결정 매트릭스 (Decision Matrix)

백엔드 시스템은 구글 캘린더 일정을 **[시간 조건]**과 **[일정 키워드/성격]**에 따라 분류한 뒤, 실시간 생체 수치(PPG)와 결합하여 최적의 호흡을 산출합니다.

#### 1️⃣ 캘린더 일정 분류 기준 (Calendar Classification Rules)
* **시간 조건**: 현재 시각 기준 `0분 ~ 30분 이내` 시작되는 일정인가? (`isUpcoming: true`)
* **일정 성격 분류 (Keyword / Event Type)**:
  * **🔥 몰입/긴장 일정 (Focus/Tension)**: 미팅, Meeting, 발표, Presentation, 면접, Interview, 시험, Exam, 회의, 데모, Pitch 등
  * **⚡ 활동/운동 일정 (Active/Energy)**: 운동, Workout, Gym, 러닝, Running, 축구, 풋살 등
  * **☯️ 일반/휴식 일정 (General/Rest)**: 점심, Lunch, 휴식, Break, 퇴근, 이동 또는 키워드가 지정되지 않은 일상 일정

---

#### 2️⃣ 생체 수치 (PPG) × 캘린더 일정 2차원 융합 의사결정 매트릭스 (2D Grid Matrix)

행(세로): **방금 측정한 생체 심박 상태(PPG)**  
열(가로): **30분 이내 캘린더 일정 성격**

| 📸 실시간 생체 상태 (PPG) \ 📅 캘린더 일정 (30분 이내) | 🔥 몰입 일정 (미팅, 발표, 면접, 시험) | ⚡ 활동 일정 (운동, 러닝) | ☯️ 일반 / 휴식 / 일정 없음 |
| :--- | :---: | :---: | :---: |
| **🚨 심박 급상승 ($\text{BPM} \ge 95$)**<br/>*(급성 긴장 / 패닉)* | 🚨 **생리학적 한숨**<br/>*(30초 긴급 진정)* | 🚨 **생리학적 한숨**<br/>*(30초 긴급 진정)* | 🚨 **생리학적 한숨**<br/>*(30초 긴급 진정)* |
| **🟢 정상 / 안정 ($60 \le \text{BPM} < 95$)**<br/>*(평온한 상태)* | 🔥 **4-4-4-4 박스 호흡**<br/>*(미팅 전 최고 몰입)* | ⚡ **4-1-2-1 각성 호흡**<br/>*(신체 에너지 부스팅)* | ☯️ **5.5-5.5 공진 호흡**<br/>*(0.1Hz 최적 자율신경 회복)* |
| **🔵 나른 / 식곤증 ($\text{BPM} < 60$)**<br/>*(저각성 상태)* | ⚡ **4-1-2-1 각성 호흡**<br/>*(미팅 전 정신 깨우기)* | ⚡ **4-1-2-1 각성 호흡**<br/>*(신체 부스팅)* | ☯️ **2-1-4-1 횡격막 복식**<br/>*(부담 없는 이완)* |

---

## 8. 🤖 AI 분석 리포트 연동 및 UX 렌더링 명세 (AI Analysis & Feedback Spec)

BPACE 앱 내 2대 핵심 UX 지점에서 사용자 맞춤형 AI 생체 분석 및 호흡 피드백 답변이 생성·렌더링됩니다.

### 1️⃣ 로그 페이지 - 분석 결과 하단 (`AI 분석 · 현재 상태`)
* **위치**: 로그 화면([`LogScreen`](file:///c:/Users/82103/Desktop/bpace/frontend/lib/screens/log_screen.dart)) 내 `분석 결과` 탭 하단
* **트리거**: 카메라 PPG 맥박 측정 완료 시 실시간 수치 업데이트
* **구조 및 디자인**:
  * **섹션 헤더**: `AI 분석 · 현재 상태`
  * **카드 디자인**: 어두운 차콜 카드 (`#28292D`, 곡률 20px)
  * **강조 제목 (요약)**: 예) *"심박수가 높아지는 순간, 리추얼이 도움이 될 수 있어요"*
  * **본문 텍스트 (4단계 분석)**:
    1. 오늘 평균 심박수 분석 (예: *"오늘의 평균 심박수는 **74 BPM**으로, 정상 범위 내에서 안정적인 상태를 유지하고 있어요."*)
    2. 최고 심박수 상승 원인 분석 (예: *"다만, 최고 심박수가 **88 BPM**까지 상승한 순간이 있었어요. 이는 일시적인 긴장이나 집중, 혹은 다가오는 일정에 대한 준비 상태로 볼 수 있어요."*)
    3. 최저 심박수 이완 상태 관찰 (예: *"최저 심박수는 **53 BPM**으로 관찰되며, 이는 리추얼 이후 이완된 상태에서 나타나는 자연스러운 수치에요."*)
    4. 총평 및 추천 가이드 (예: *"전반적인 컨디션은 양호한 편이며, 심박수가 높아지는 순간엔 **짧은 리추얼로 미리 준비해보는 걸 추천드려요.**"*)
  * **의료 면책 안내문구 (Fixed Disclaimer)**:
    *"이 결과는 의료 진단·치료·응급 판단을 위한 정보가 아닌 웰빙 참고용입니다. 심각한 증상이나 응급 상황이라면 의료기관 또는 119에 연락하세요."*

---

### 2️⃣ 호흡 피드백 화면 하단 (`AI 분석 · 피드백`)
* **위치**: 호흡 세션 완주 직후 나타나는 피드백 화면([`BreathingCompletionScreen`](file:///c:/Users/82103/Desktop/bpace/frontend/lib/screens/breathing_completion_screen.dart)) 하단
* **트리거**: 고정 호흡 및 맞춤 호흡 세션 완주 시 자동 생성
* **구조 및 디자인**:
  * **섹션 헤더**: `AI 분석 · 피드백`
  * **카드 디자인**: 어두운 차콜 카드 (`#28292D`, 곡률 20px)
  * **강조 제목 (요약)**: 예) *"심박수가 높아지는 순간, 리추얼이 도움이 될 수 있어요"*
  * **수행 요약 강조문**: 예) *"**0분 1초 동안 1번의 호흡**을 마쳤어요."*
  * **맞춤 피드백 본문**: 예) *"4-6 릴렉스 호흡은 긴장을 천천히 가라앉히는 데 효과적인 리듬으로 알려져 있어요. 시작 전 컨디션이 78점으로 이미 안정적인 편이었는데, 이번 Ritual로 그 흐름을 한 번 더 다듬은 셈이에요."*
  * **의료 면책 안내문구 (Fixed Disclaimer)**:
    *"이 결과는 의료 진단·치료·응급 판단을 위한 정보가 아닌 웰빙 참고용입니다. 심각한 증상이나 응급 상황이라면 의료기관 또는 119에 연락하세요."*
  * **하단 액션 버튼**: `Ritual 기록 저장` (라이트 민트 그린 알약 버튼)

---

> 💡 본 고도화 명세서는 프론트엔드 호흡 알고리즘 및 UX 사용자 경험 흐름과 100% 동기화된 **BPACE 공식 기술·학술 연구 명세서**입니다.
